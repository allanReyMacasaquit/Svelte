<footer
    class="py-20 sm:py-20 bg-slate-900 border-t border-solid rounded-lg border-blue-600 flex flex-col gap-4 sm:gap-8 justify-center items-center"
>
    <p class="px-4 py-2 bg-white text-slate-950 font-medium">
        Connect with me &darr;
    </p>
    <div class="flex flex-col gap-4 items-center justify-center">
        <div
            class="flex items-center gap-4 sm:gap-6 text-2xl sm:text-3xl md:text-4xl mx-auto"
        >
            <a href='/'>
                <i class="fa-brands fa-linkedin-in" />
            </a>
            <a href='/'>
                <i class="fa-brands fa-github" />
            </a>
        </div>
        <p class="mx-auto text-sm opacity-70">or</p>
        <p class="">amacasaquit</p>
        <p><b class="pr-2">Email</b> allanrey.macasaquit@gmail.com</p>
        <p>
            <b class="pr-2">GitHub</b>
            <a href=" " target="_blank" class="text-blue-400"
                >amacasaquit<sup class=""
                    ><span class="text-xs scale-75 pl-0.5"
                        ><i
                            class="fa-solid fa-arrow-up-right-from-square text-xs scale-[75%]"
                        /></span
                    ></sup
                ></a
            >
        </p>
        <p>
            <b class="pr-2">LinkedIn</b>
            <a href=" " target="_blank" class="text-blue-400"
                >amacasquit<sup class=""
                    ><span class="text-xs scale-75 pl-0.5"
                        ><i
                            class="fa-solid fa-arrow-up-right-from-square text-xs scale-[75%]"
                        /></span
                    ></sup
                ></a
            >
        </p>
    </div>
</footer>